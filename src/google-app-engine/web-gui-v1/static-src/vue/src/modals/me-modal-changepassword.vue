<template>
    <me-modal id='modal_change_password' ref='modal' title='Change your password' v-on:hidden='reset'>
        <form class='ui form'>
            <me-input :disabled='saving' :error='current_pw_error' icon='lock' id='current_pw' label='Current password' type='password' v-model='fields.current_pw' v-on:enter='submit'></me-input>
            <me-input :disabled='saving' :error='new_pw_1_error' icon='lock' id='new_pw_1' label='New password' type='password' v-model='fields.new_pw_1' v-on:enter='submit'></me-input>
            <me-input :disabled='saving' :error='new_pw_2_error' icon='lock' id='new_pw_2' label='Repeat new password' type='password' v-model='fields.new_pw_2' v-on:enter='submit'></me-input>
        </form>
        <template v-slot:actions>
            <me-button :disabled='saving' v-on:click='close'>Cancel</me-button>
            <me-button :disabled='saving' :loading='saving' label_icon='lock' label_position='right' primary v-on:click='submit'>Change password</me-button>
        </template>
    </me-modal>
</template>
...
export default {
    ...
    methods: {
        ...
        validate: function() {
            ...
            if (
                this.fields.current_pw == null ||
                this.fields.current_pw == ''
            ) {
                this.current_pw_error = true;
            } else {
                this.current_pw_error = false;
            }
            if (this.fields.new_pw_1 == null || this.fields.new_pw_1 == '') {
                this.new_pw_1_error = true;
            } else {
                this.new_pw_1_error = false;
            }
            if (this.fields.new_pw_2 == null || this.fields.new_pw_2 == '') {
                this.new_pw_2_error = true;
            } else {
                this.new_pw_2_error = false;
            }
            if (this.fields.new_pw_1 != this.fields.new_pw_2) {
                this.new_pw_1_error = true;
                this.new_pw_2_error = true;
            }
        },
                    ...
                    .then(function(data) {
                        if (data.data.success) {
                            ...
                            vue_this.$store.commit(
                                'api_save_user_object_password_date'
                            );
                        } else {
