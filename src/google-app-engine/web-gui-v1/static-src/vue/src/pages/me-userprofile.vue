    ...
    <div id='me-content-userprofile'>
        <me-page-title icon='user circle'>
            ...
            <template v-slot:actions>
                <me-button :disabled='!changed' :loading='saving' primary v-on:click='save_profile'>Save</me-button>
                <me-button :disabled='!changed' v-on:click='set_field_values'>Cancel</me-button>
            </template>
        </me-page-title>
        <me-grid>
            <me-cell class='tablet-span-8' padding v-bind:span='4'>
                <me-card raised wide>
                    <me-h1 inverted>User profile</me-h1>
                    <form class='ui form'>
                        <me-input :disabled='saving' :error='fields.username_error' icon='user' id='username' label='Username' placeholder='Username' v-model='user_object.username' v-on:enter='save_profile' v-on:input='changed = true'></me-input>
                        <me-input :disabled='saving' :error='fields.fullname_error' icon='user circle' id='fullname' label='Full name' placeholder='Full name' v-model='user_object.fullname' v-on:enter='save_profile' v-on:input='changed = true'></me-input>
                        <me-input :disabled='saving' :error='fields.email_error' icon='user' id='email' label='E-mail address' placeholder='E-mail address' v-model='user_object.email' v-on:enter='save_profile' v-on:input='changed = true'></me-input>
                    </form>
                </me-card>
            </me-cell>
            <me-cell padding v-bind:span='4'>
                <me-card raised wide>
                    ...
                    <div class='action_button'>
                        <div v-if='password_age == 1'>
                            Your password is
                            <b>{{ password_age }} day</b> old.
                        </div>
                        <div v-if='password_age > 1'>
                            Your password is
                            <b>{{ password_age }} days</b> old.
                        </div>
            ...
            <me-cell padding v-bind:span='4'>
                <me-card raised wide>
                    ...
                    <div class='action_button'>
                        <div v-if='$store.state.api_data.user_token_object.description'>
                            Your session is named
                            <b>{{ $store.state.api_data.user_token_object.description }}</b>
                        </div>
                        <div v-if='!$store.state.api_data.user_token_object.description'>
                            This session has
                            <b>no name</b>
                        ...
                        <div>
                            <me-button v-on:click='rename_session'>Rename session</me-button>
                        </div>
                    </div>
                    <div class='action_button'>
                        <div v-if='$store.state.api_data.user_token_object.expiration'>
                            Your session will expire in
                            <b>{{ user_session_expire_period }}</b>
                        </div>
                        <div v-if='!$store.state.api_data.user_token_object.expiration'>This session will not expire</div>
                        <div>
                            <me-button v-bind:disabled='!user_session_refreshable' v-bind:loading='loading_refresh' v-on:click='refresh_token'>Refresh session</me-button>
                        </div>
                    </div>
                </me-card>
            </me-cell>
        ...
        <me-page-title icon='user circle'>
            GUI settings
            <template v-slot:actions>
                <me-button :disabled='!user_settings_changed' :loading='saving_user_settings' primary v-on:click='save_user_settings'>Save</me-button>
                <me-button :disabled='!user_settings_changed' v-on:click='set_user_settings_field_values'>Cancel</me-button>
            </template>
        </me-page-title>
        <me-grid>
            <me-cell class='tablet-span-8' padding v-bind:span='4'>
                <me-card class='date_formats' raised wide>
                    <me-h1 inverted>Date and time formats</me-h1>
                    <form class='ui form'>
                        <div class='setting_field'>
                            <me-input :disabled='saving_user_settings' icon='clock outline' id='datetime_format' label='Datetime format' placeholder='Datetime format' v-model='user_settings.datetime_formats.datetime_format' v-on:input='user_settings_changed = true'></me-input>
                            <p class='example_title'>Example</p>
                            <div class='dateformat_example'>{{ example_datetime_format }}</div>
                        </div>
                        <div class='setting_field'>
                            <div class='ui slider checkbox'>
                                <input :disabled='saving_user_settings' name='24h' type='checkbox' v-model='user_settings.datetime_formats.show_24h' v-on:input='user_settings_changed = true' />
        ...
        <me-page-title icon='key'>
            User tokens
            <template v-slot:actions>
                <me-button primary v-bind:disabled='!loaded_clients' v-on:click='set_clients_forced'>Reload</me-button>
            </template>
        </me-page-title>
        <me-grid>
            <me-cell padding v-bind:span='12'>
                <me-card class='clients' raised wide>
                    ...
                    <me-userprofile-api-client v-bind:client='client' v-bind:key='client.id' v-for='client in $store.state.api_data.api_clients.clients' v-if='loaded_clients'></me-userprofile-api-client>
...
export default {
    ...
    computed: {
        user_session_expire_period: function() {
            let today = new Date();
            let difference =
                (this.$store.state.api_data.user_token_object.expiration.getTime() -
                    today.getTime()) /
                1000;
        ...
        password_age: function() {
            ...
            let pw_date = new Date(
                this.$store.state.api_data.user_object.password_date.toDateString()
            );
            ...
            refresh_user_token({
                ...
                failed: function() {
                    ...
                    $('body').toast({
                        position: 'bottom center',
                        message:
                            'Something went wrong while refreshing your usersession',
                        closeIcon: true,
                        displayTime: 'auto',
                        showIcon: 'user',
                        class: 'error'
                ...
                this.$store.commit('api_save_user_object', {
                    ...
                    failed: function(error) {
                        ...
                        if (error == 'username_in_use') {
                            ...
                            vue_this.fields.username_error = true;
                        }
                        if (error == 'username_invalid') {
                            error_text = 'username is not valid';
                            vue_this.fields.username_error = true;
                        }
                        if (error == 'fullname_invalid') {
                            error_text = 'full name is not valid';
                            vue_this.fields.fullname_error = true;
                        }
                        if (error == 'email_in_use') {
                            error_text = 'e-mailaddress is already in use';
                            vue_this.fields.email_error = true;
                        }
                        if (error == 'email_invalid') {
                            error_text = 'e-mailaddress is invalid';
                            vue_this.fields.email_error = true;
                        }
                        ...
                        $('body').toast({
                            position: 'bottom center',
                            message:
                                "Couldn't save your userprofile; " + error_text,
                            closeIcon: true,
                            displayTime: 'auto',
                            showIcon: 'user',
                            class: 'error'
        ...
        set_user_settings_field_values: function() {
            ...
            this.$store.commit('update_user_settings', {
                success: function(data) {
                    ...
                    vue_this.user_settings.datetime_formats.show_24h =
                        data.datetime_formats['show_24h'];
                    vue_this.user_settings.datetime_formats.datetime_format =
                        data.datetime_formats.datetime_format;
                },
                failed: function() {
                    $('body').toast({
                        position: 'bottom center',
    ...
    beforeRouteLeave: function(to, from, next) {
        ...
        if (this.changed || this.user_settings_changed) {
            $('body').toast({
                message:
                    "You haven't saved the form yet. Are you sure you want to leave?",
                displayTime: 0,
                class: 'white',
                position: 'top center',
